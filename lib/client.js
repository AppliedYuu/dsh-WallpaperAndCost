window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function onFileChosen(file, onError) {
      if (typeof FileReader === 'undefined') { if (onError) onError('当前环境不支持本地上传'); return; }
      try {
        const reader = new FileReader();
        reader.onload = () => {
          editor.set({ open: true, id: null, pending: { dataUrl: reader.result, name: file.name || '图片', w: 0, h: 0 } });
        };
        reader.onerror = () => { if (onError) onError('读取图片失败'); };
        reader.readAsDataURL(file);
      } catch (e) { if (onError) onError('读取图片失败'); }
    }
    ...
    function SettingsPage() {
      ...
      const uploadNode = (u) => React.createElement('div', { key: u.id, style: { position: 'relative' } },
      ...
      return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 14, padding: '4px 0 16px' } },
        ...
        (s.uploads.length > 0) && React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 6 } },
          React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary, #999)' } }, '我的上传（原图不压缩 · 点 👁 预览/调整裁剪范围）'),
        ...
        React.createElement('div', { style: { display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' } },
          React.createElement('label', { style: { ...btnStyle, display: 'inline-flex', alignItems: 'center', gap: 6 } },
            '上传图片',
