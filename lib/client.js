window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function captureVideoThumb(item, onDone) {
      ...
      try {
        const v = document.createElement('video');
        v.preload = 'metadata';   // 轻量优先: 只取首帧; 取不到再升级全量加载
        v.muted = true;
        v.playsInline = true;
        let finished = false;
        let retried = false;
        ...
        v.addEventListener('loadeddata', () => {
          try { if (v.readyState >= 2) v.currentTime = 0.01; } catch (e) { draw(); }
        });
        v.addEventListener('seeked', draw);
        v.addEventListener('timeupdate', () => { try { if (v.currentTime > 0) draw(); } catch (e) {} });
        // 兜底: metadata 模式 2.5s 内取不到帧, 升级为全量加载再截
    ...
    function WallpaperImportModal() {
      ...
      const scanRoot = async (dir) => {
        ...
        try {
          ...
          for await (const [name, handle] of dir.entries()) {
            ...
            let kind = '未知', preview = null;
            try {
              for await (const [fname, fh] of handle.entries()) {
                if (fname === 'scene.pkg') kind = '场景';
                else if (/\.mp4$/i.test(fname)) kind = '视频';
                else if (/^wallpaper\.(jpg|jpeg|png)$/i.test(fname)) kind = '图片';
                else if (fname === 'preview.jpg' && !preview) {
                  try { const f = await fh.getFile(); preview = URL.createObjectURL(f); } catch (e) {}
                }
              }
            } catch (e) {}
            list.push({ name, handle, kind, preview });
          }
      ...
      const extractFromDir = async (dir) => {
        ...
        if (direct) {
          ...
          return { best: { name: direct, blob: f, kind: /\.(mp4|webm)$/i.test(direct) ? 'mp4' : 'jpg' } };
        }
        // 2) scene.pkg
        if (files['scene.pkg']) {
          ...
          if (pf.size <= WS_MAX_PKG) {
            ...
            const best = pickBestMedia(media);
            if (best) {
              const blob = new Blob([best.data], { type: mimeOf(best.kind) });
              return { best: { name: 'wallpaper.' + best.kind, blob, kind: best.kind } };
            }
          }
        }
        // 3) 候选: preview.jpg + 文件夹内全部文件
        const fileList = [];
        for (const n of Object.keys(files)) {
          try {
            const f = await files[n].getFile();
            fileList.push({ name: n, size: f.size, handle: files[n] });
          } catch (e) {}
        }
        return { best: null, files: fileList, hasPreview: !!files['preview.jpg'] };
      };
      ...
      const importItem = async (item) => {
        ...
        try {
          ...
          if (result.best) {
            ...
            onFileChosen(file, (m) => setMsg(m), { maxBytes: WS_MAX_MEDIA });
            closeImportModal();
          } else {
