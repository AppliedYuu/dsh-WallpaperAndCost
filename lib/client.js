window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── better-sidebar 联动检测（动态）──────────────────────────────────
    // 原来在一次模块加载时就把 HAS_BETTER_SIDEBAR 拍死成常量：当前 DSH web
    // 运行时已没有 __DSH_MODULES__ 全局，且本插件客户端先于 better-sidebar
    // 加载（boot 顺序里本插件在前），求值那一刻 better-sidebar 的样式标签和
    // 面板 DOM 都还不存在 → 检测恒为 false，右侧边栏滑杆与联动 CSS 全部失效。
    // 现在改为惰性检测：在应用壁纸/渲染设置页/观察器回调时实时判断。
    //
    // better-sidebar 注入样式时同时设置 data-plugin 与 data-plugin-css 属性；
    // 面板挂载在 [data-dsh-panel-host] 内（.nArs4W_panel 等为 CSS-module
    // 哈希类名，跨版本会变，只能作辅助信号，不能硬依赖）。
    function hasBetterSidebarNow() {
      try {
        const m = globalThis.__DSH_MODULES__;
        if (m) {
          if (m.graphRows && m.graphRows.has && m.graphRows.has('dsh-better-sidebar')) return true;
          if (m.factories && m.factories.has && m.factories.has('dsh-better-sidebar')) return true;
          if (m.loadCache && m.loadCache.has && m.loadCache.has('dsh-better-sidebar')) return true;
        }
      } catch (e) {}
      try {
        if (typeof document !== 'undefined') {
          if (document.querySelector('style[data-plugin="dsh-better-sidebar"]')) return true;
          if (document.querySelector('style[data-plugin-css^="dsh-better-sidebar/"]')) return true;
          if (document.querySelector('[data-dsh-panel-host]')) return true;
          if (document.querySelector('.nArs4W_panel')) return true;
        }
      } catch (e) {}
      return false;
    }
    // 响应式状态: 设置页滑杆跟着它走; 翻转时强制重刷壁纸样式(指纹里带检测位)。
    const sidebarStore = makeStore({ better: false });
    function refreshSidebarDetection(rebuild = true) {
      const v = hasBetterSidebarNow();
      const st = sidebarStore.get();
      if (st.better !== v) {
        sidebarStore.set({ better: v });
        if (rebuild) {
          try { applyWallpaper(store.get()); } catch (e) {}
        }
      }
      return v;
    }
    ...
    function buildRightRule(opRightRaw) {
      ...
      // 核心三栏的右侧列 (detailsCol) 始终需要覆盖; better-sidebar 的部分按实时
      // 检测追加 (含哈希无关的 [data-dsh-panel-host] 面板兜底选择器)。
      if (or <= 0) {
        let css = '[class*="detailsCol"]{background:transparent!important}';
        if (hasBetterSidebarNow()) {
          css += '.nArs4W_panel,.nArs4W_bottomPanel{background:transparent!important}'
            + '[data-dsh-panel-host] > [class*="_panel"]{background:transparent!important}';
        }
        return css;
      }
      const light = 'rgba(244,246,250,' + or.toFixed(3) + ')';
      const dark = 'rgba(16,18,24,' + or.toFixed(3) + ')';
      let css = '[class*="detailsCol"]{background:' + light + '!important}body[data-ds-dark-theme] [class*="detailsCol"]{background:' + dark + '!important}';
      if (hasBetterSidebarNow()) {
        css += '.nArs4W_panel,.nArs4W_bottomPanel{background:' + light + '!important}'
          + 'body[data-ds-dark-theme] .nArs4W_panel,body[data-ds-dark-theme] .nArs4W_bottomPanel{background:' + dark + '!important}'
          + '[data-dsh-panel-host] > [class*="_panel"]{background:' + light + '!important}'
          + 'body[data-ds-dark-theme] [data-dsh-panel-host] > [class*="_panel"]{background:' + dark + '!important}';
      }
      return css;
    }
    ...
    // 设置面板是否打开。壁纸设置页(settings.section, list)在设置面板打开时挂载、
    // 关闭时卸载——用这个生命周期暂停/恢复动态视频壁纸, 释放 GPU。
    let settingsOpen = false;

    function pauseVideoWallpaper() {
      try {
        const v = document.querySelector('video#dsh-wallpaper-video');
        if (v && !v.paused) v.pause();
      } catch (e) {}
    }
    function resumeVideoWallpaper() {
      try {
        const v = document.querySelector('video#dsh-wallpaper-video');
        if (v && v.paused) {
          const p = v.play();
    ...
    function applyVideoWallpaper(u) {
      ...
      if (settingsOpen) pauseVideoWallpaper(); // 设置面板开着时新建的视频壁纸也保持暂停
    ...
    function applyWallpaper(next) {
      const fp = wallpaperFingerprint(next) + (hasBetterSidebarNow() ? '#R' : '#N');
    ...
    function SettingsPage() {
      ...
      const hasSide = useStore(sidebarStore);
      const [hint, setHint] = React.useState('');
      // 设置面板打开期间 → 暂停动态视频壁纸, 释放 GPU; 关闭恢复播放
      React.useEffect(() => {
        settingsOpen = true;
        pauseVideoWallpaper();
        return () => { settingsOpen = false; resumeVideoWallpaper(); };
      ...
      return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 14, padding: '4px 0 16px' } },
        ...
        React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 8 } },
          ...
          hasSide.better && sliderRow('右侧边栏', s.opRight, setOpRight)),
    ...
    function apply(ctx) {
      ...
      // ---- 右侧边栏(better-sidebar)联动: 实时检测 ----
      // better-sidebar 的样式标签在其客户端模块加载时注入, 面板按会话挂载,
      // 都在本插件加载之后 —— 不能用加载时的一次性常量。监听 document 子树,
      // 样式标签/面板出现或消失时即时刷新滑杆可见性与联动 CSS。
      try {
        if (typeof MutationObserver !== 'undefined' && typeof document !== 'undefined' && document.documentElement) {
          const observer = new MutationObserver(() => { try { refreshSidebarDetection(true); } catch (e) {} });
          observer.observe(document.documentElement, { subtree: true, childList: true });
          ctx.effect(() => () => { try { observer.disconnect(); } catch (e) {} }, "merged: sidebar-linkage observer");
        }
      } catch (e) {}
      try { refreshSidebarDetection(false); } catch (e) {} // 先刷新状态, 再让 loadPersisted 的首个 commit 带上右侧规则
      loadPersisted();
    }
